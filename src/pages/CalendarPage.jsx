...
function SelectedDay({date,settings,events,openSource}) {
  ...
  return <aside className="selected-day"><p className="eyebrow">היום שנבחר · <LtrDate value={date} /></p><h2>{hebrewDateLabel(date)}</h2>{events.filter(e=>e.category!=='hebdate').map((e,i)=><div className="event-line" key={i}><strong>{hebrewEventLabel(e.hebrew||e.title)}</strong>{e.date.includes('T')&&<time>{timeLabel(e.date,settings.location.tzid)}</time>}{e.leyning?.torah&&<button onClick={()=>openSource(e.leyning.torah)}>קריאת התורה ↗</button>}</div>)}<ResourceState resource={solar}/><details open><summary>זמני היום</summary>{ZMANIM.filter(([k])=>k!=='tzeit72min'||settings.showRT).map(([key,name,method])=><div className="compact-time" key={key}><span title={method}>{name}</span><time>{timeLabel(solar.data?.[key],settings.location.tzid)}</time></div>)}</details></aside>;
}
