...
import { VERSE_INDEX_SIZE, findNameVerses, formatGregorian, hebrewFromGregorian, hebrewFromParts, hebrewMonthsForYear, loadPersonalProfile, nameLetters, parashaForDate, parseGregorian, savePersonalProfile, shareText } from '../services/personalTools.mjs';
...
const field = (label, value, onChange, props = {}) => <label className="personal-field"><span>{label}</span><input {...props} value={value} onChange={e => onChange(e.target.value)} /></label>;
...
function DateConverter() {
  ...
  const currentHebrew = useMemo(() => hebrewFromGregorian(parseGregorian(civil.day, civil.month, civil.year)), [civil]);
  const [hebrew, setHebrew] = useState(() => ({ day: currentHebrew.day, month: currentHebrew.month, year: currentHebrew.year }));
  const [result, setResult] = useState(null);
  const [error, setError] = useState('');
  const updateCivil = (key, value) => setCivil(previous => ({ ...previous, [key]: value }));
  const updateHebrew = (key, value) => setHebrew(previous => ({ ...previous, [key]: value }));
  const convert = event => {
