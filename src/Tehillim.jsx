  ...
  return (
    <div style={{ padding: 14, direction: 'rtl' }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginBottom: 10 }}>
        ...
        <button onClick={() => setChapter(c => Math.max(1, c - 1))} style={btn(T, false)}>→ קודם</button>
        ...
        <button onClick={() => setChapter(c => Math.min(150, c + 1))} style={btn(T, false)}>הבא ←</button>
      ...
      {hits.length > 0 && <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 10 }}>
        {hits.slice(0, 12).map(p => <button key={p.chapter} onClick={() => { setChapter(p.chapter); setQ(''); }} style={btn(T, p.chapter === chapter)}>{p.title}</button>)}
      </div>}
      {favorites.length > 0 && <p style={{ color: T.muted, fontSize: 12, marginBottom: 10 }}>מועדפים: {favorites.slice().sort((a, b) => a - b).map((c, i) =>
        <button key={i} onClick={() => setChapter(c)} style={{ ...btn(T, false), marginLeft: 4 }}>{c}</button>)}</p>}
      {!data && !error && <p className="notice">טוען טקסט מנוקד…</p>}
      {error && <p className="notice error">{error}</p>}
      {verses && (
        <article style={{ fontSize: font, lineHeight: 1.9, color: T.text, background: T.card, border: '1px solid ' + T.border, borderRadius: 12, padding: '18px 16px' }}>
