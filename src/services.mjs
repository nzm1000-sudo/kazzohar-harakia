...
export function calendarRequestKey(start, end, settings) {
  ...
  const candles = settings.candles ?? 20;
  return `${start}|${end}|${settings.location.latitude}|${settings.location.longitude}|${settings.location.tzid}|${residence}|${candles}`;
}
...
export function getNextRelevantZman(now = new Date(), zmanim = {}, { showRT = false } = {}) {
  ...
  const candidates = ZMANIM
    ...
    .concat(Object.entries(zmanim?.nextDay || {})
      .filter(([key]) => key !== 'tzeit72min' || showRT)
      .filter(([key]) => labels.has(key))
      .map(([key, at]) => ({ key, ...labels.get(key), at })));
  return candidates
