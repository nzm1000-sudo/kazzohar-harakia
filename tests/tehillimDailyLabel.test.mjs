...
const files = dir => readdirSync(dir).flatMap(name => { const path = join(dir, name); return statSync(path).isDirectory() ? (name === 'data' || name === '.kilo' ? [] : files(path)) : /\.(jsx|mjs)$/.test(name) ? [path] : []; });
